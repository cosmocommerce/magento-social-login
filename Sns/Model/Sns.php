...
class CosmoCommerce_Sns_Model_Sns extends Mage_Core_Model_Abstract
{
	public $client_id;
    ...
    public function setKey($client_id = NULL, $client_secret = NULL, $access_token = NULL, $refresh_token = NULL)
    {
	...
	public function getAccessToken( $type = 'code', $keys ) {
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			echo ("get access token failed." . $token['error']);
			exit();
		}
		return $token;
	}
	 ...
	 * @return mixed
	 */
	public function deletex($url, $parameters = array()) {
	...
	public function getPaymentGetTokenUrl(){
		$payment_sandbox=Mage::getStoreConfig('sns/sns_weibo/payment_sandbox');
		if($payment_sandbox){
			return "http://pay.api.weibo.com/test/pay/get_token.json";
		}else{
			return "http://pay.api.weibo.com/pay/get_token.json";
		}
	}

	public function getPaymentPayUrl(){
		$payment_sandbox=Mage::getStoreConfig('sns/sns_weibo/payment_sandbox');
		if($payment_sandbox){
			return "http://open.weibo.com/paytest/payTestPay.php";
		}else{
			return "http://pay.weibo.com/wb/?c=pay";
		}
	}
	public function pay_get_token( $order_id, $amount,$desc,$access_token,$app_secret )
	{
		$params = array();
		$params['order_id'] = $order_id;
		$params['amount'] = $amount;
		$params['desc'] = $desc;
		$params['sign'] = md5($order_id.'|'.$amount.'|'.$desc.'|'.$app_secret);
		$params['access_token'] = $access_token;
		return $this->get( $this->getPaymentGetTokenUrl(), $params);
